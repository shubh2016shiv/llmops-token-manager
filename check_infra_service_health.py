...
def check_rabbitmq() -> Tuple[bool, str]:
    ...
    try:
        ...
        response = requests.get(
            url,
            auth=(rmq_config["user"], rmq_config["password"]),
            timeout=5
        )
...
def check_docker_compose_file() -> bool:
    ...
    try:
        with open("docker-compose.yml", "r") as f:
            return "postgres:" in f.read() and "redis:" in f.read() and "rabbitmq:" in f.read()
...
def main():
    ...
    if pg_running:
        ...
        pg_conn = f"psycopg2.connect(host='{SERVICES['PostgreSQL']['host']}', " \
    ...
    if redis_running:
        ...
        redis_conn = f"redis.Redis(host='{SERVICES['Redis']['host']}', " \
                    f"port={SERVICES['Redis']['port']})"
    ...
    if rmq_running:
        ...
        rmq_conn = f"AMQP: amqp://{SERVICES['RabbitMQ']['user']}:****@{SERVICES['RabbitMQ']['host']}:{SERVICES['RabbitMQ']['amqp_port']}/\n" \
                  f"Management: http://{SERVICES['RabbitMQ']['host']}:{SERVICES['RabbitMQ']['mgmt_port']}"
