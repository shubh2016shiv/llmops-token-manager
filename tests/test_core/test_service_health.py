...
class TestDisplayFunctions:
    ...
    @patch("app.core.service_health.settings")
    ...
    @patch("app.llm_client_provisioning.service_health.settings")
    def test_display_provisioning_service_info_sections(self, mock_settings):
        """Test provisioning runtime startup display sections."""
        ...
        try:
            ...
            output = captured_output.getvalue()
            assert "PROVISIONING RUNTIME" in output
            assert "RABBITMQ BROKER" in output
            assert "CELERY WORKER READINESS" in output
            assert "rpc://" in output
            assert "llm_requests_quorum_v1" in output
            assert "priority_requests_quorum_v1" in output
        finally:
            sys.stdout = original_stdout
...
class TestVerifyDatabaseConnectivity:
    ...
    @pytest.mark.asyncio
    @patch("app.core.service_health.db_manager")
    @patch("app.core.service_health.settings")
...
class TestVerifyRedisConnectivity:
    ...
    @pytest.mark.asyncio
    @patch("app.core.service_health.redis_manager")
    @patch("app.core.service_health.settings")
...
class TestVerifyCeleryWorkerReadiness:
    ...
    @pytest.mark.asyncio
    @patch("app.llm_client_provisioning.service_health.settings")
    @patch("app.llm_client_provisioning.service_health.inspect_celery_worker_readiness")
